...
import { chatWithOnlineAi, type ChatMessage } from "@/lib/ai";
...
interface AskMarkSidebarContentProps {
  selectedText?: string;
  noteTitle?: string;
  noteContent?: string;
...
export function AskMarkSidebarContent({
  selectedText = "",
  noteTitle = "Nota Sem Título",
  noteContent = "",
  onInsertResult,
}: AskMarkSidebarContentProps) {
  ...
  const handleSendMessage = async (userPrompt?: string) => {
    ...
    try {
      ...
      setMessages([...newMessages, { role: "assistant", text: cleanMarkdownFences(responseText) }]);
    } catch (err: any) {
  ...
  const handleCopy = (text: string) => {
    navigator.clipboard.writeText(cleanMarkdownFences(text));
  };

  const handleInsert = (text: string, replaceSelection: boolean) => {
    if (onInsertResult) {
      onInsertResult(cleanMarkdownFences(text), replaceSelection);
    }
  };
  ...
  return (
    <div className="flex h-full flex-col bg-bg">
          ...
          >
            ...
            {msg.role === "assistant" && (
              <div className="flex items-center gap-2 pt-1 text-[11px] text-faint">
                ...
                {selectedText && onInsertResult && (
                  <button
                    type="button"
                    onClick={() => handleInsert(msg.text, true)}
                ...
                {onInsertResult && (
                  <button
                    type="button"
                    onClick={() => handleInsert(msg.text, false)}
      ...
      {/* Input Box */}
      <div className="border-t border-line p-3 bg-panel/50 space-y-2">
        <div className="flex items-center gap-2">
          <textarea
            ...
            placeholder="Pergunte ou peça algo ao Mark..."
            rows={2}
            className="w-full resize-none rounded-xl border border-line bg-bg p-2.5 text-xs text-ink placeholder:text-faint focus:border-purple-500 focus:outline-hidden"
          />
        </div>
        <div className="flex items-center justify-between text-[11px] text-faint">
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input
              ...
              className="rounded-xs border-line text-purple-600 focus:ring-purple-500"
            />
            <span>Enviar nota como contexto</span>
          </label>
          <button
            type="button"
            onClick={() => handleSendMessage()}
            disabled={loading || !inputText.trim()}
            className="inline-flex items-center gap-1.5 rounded-xl bg-purple-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-purple-700 disabled:opacity-50 transition-colors"
          >
